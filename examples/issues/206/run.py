...
class DxtbAseCalculator(AseCalculator):
    ...
    def calculate(
        ...
        # With cache enabled, the energy is not calculated again
        energy = dxtb_calculator.get_energy(positions, chrg=chrg)

        # Without caching, either...
        # 1. Resetting is needed:
        #
        # energy = dxtb_calculator.get_energy(positions, chrg=chrg)
        # dxtb_calculator.reset()
        # positions.requires_grad_(True)
        # forces = dxtb_calculator.get_forces(positions, chrg=chrg)
        #
        # 2. Or, the forces are directly calculated:
        # positions.requires_grad_(True)
        # energy = dxtb_calculator.get_energy(positions, chrg=chrg)
