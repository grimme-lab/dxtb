...
from xtbml.param.gfn1 import GFN1_XTB, Param
...
def test_param_calculator(dtype: torch.dtype) -> None:
    ...
    calc = Calculator(numbers, dummy_coords, GFN1_XTB)
...
def test_export() -> None:
    with TemporaryDirectory() as td:
        path = Path(td, "gfn1_xtb.toml")
        GFN1_XTB.to_toml(path)

        # read in again
        with open(path, "rb") as fd:
            par = Param(**toml.load(fd))
            assert par == GFN1_XTB

        # try writing same file again
        with pytest.raises(FileExistsError):
            GFN1_XTB.to_toml(path, overwrite=False)
