...
def get_param_tensor_from_dict(
    ...
    """
    return torch.tensor(
        [l for number in numbers for l in d.get(number.item(), [])],
        dtype=dtype,
    )
...
class H0:
    ...
    def build(self, ovlp, cn: Tensor | None = None) -> Tensor:
        ...
        # Eq.29: H_(mu,mu)
        # ----------------
        selfenergy = self.ihelp.spread_ushell_to_orbital(self.selfenergy)
        if cn is not None:
            cn = self.ihelp.spread_atom_to_orbital(cn)
            kcn = self.ihelp.spread_ushell_to_orbital(self.kcn)
        ...
        positions = batch.index(
            batch.index(self.positions, self.ihelp.shells_to_atom),
            self.ihelp.orbitals_to_shell,
        )
        distances = torch.cdist(positions, positions, p=2)
        rad = self.ihelp.spread_uspecies_to_orbital(self.rad)
        rr = torch.sqrt(distances / (rad.unsqueeze(-1) + rad.unsqueeze(-2)))

        shpoly = self.ihelp.spread_ushell_to_orbital(self.shpoly)
        ...
        # Eq.28: X(EN_A, EN_B)
        # --------------------
        en = self.ihelp.spread_uspecies_to_orbital(self.en)
        var_x = 1.0 + self.par.hamiltonian.xtb.enscale * torch.pow(
            en.unsqueeze(-1) - en.unsqueeze(-2), 2.0
        )

        # --------------------
        # Eq.23: K_{AB}^{l,l'}
        # --------------------
        hscale = self.ihelp.spread_ushell_to_orbital(self.hscale, dim=(-2, -1))
        kpair = self.ihelp.spread_ushell_to_orbital(self.kpair, dim=(-2, -1))
        valence = self.ihelp.spread_ushell_to_orbital(self.valence)
        ...
        # Eq.23: H_EHT
        # ------------
        selfenergy = 0.5 * (selfenergy.unsqueeze(-1) + selfenergy.unsqueeze(-2))

        mask = torch.ones(var_k.shape, dtype=torch.bool)
        mask.diagonal(dim1=-2, dim2=-1).fill_(False)

        # TODO
        var_s = ovlp

        return torch.where(mask, var_s * var_pi * var_k * selfenergy, selfenergy)
