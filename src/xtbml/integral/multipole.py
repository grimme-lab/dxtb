...
# TODO: SE -> merge these configs into single file
# TODO: MF -> how it currently is?
from ..exlibs.config import dimDipole, dimQuadrupole
...
def multipole_cgto(cgtoj, cgtoi, r2, vec, intcut, overlap, dpint, qpint):
    ...
    for mli in range(msao[cgtoi.ang]):
        for mlj in range(msao[cgtoj.ang]):
            tr = 0.5 * (
                qpint[1, mlj, mli]
                + qpint[dimDipole, mlj, mli]
                + qpint[dimQuadrupole, mlj, mli]
            )
...
def multipole_3d(rpj, rpi, aj: float, ai: float, lj, li, s1d, s3d, d3d, q3d):
    ...
    for k in range(3):
        ...
        for l in range(li[k] + lj[k] + 1):
            ...
            v1d[k, 2] += (
                s1d[l + 2] + 2 * rpi[k] * s1d[l + 1] + rpi[k] * rpi[k] * s1d[l]
            ) * vv[l]
