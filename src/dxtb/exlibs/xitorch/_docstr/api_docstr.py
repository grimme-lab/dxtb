...
def get_methods_docstr(
    ...
    * full_docstr: str
    ...
    if isinstance(methods, dict):
        generator = (
            methods.items()
        )  # type: Union[ItemsView[str, Any], Generator[Tuple[str, Any], None, None]]
    ...
    for name, method in generator:
        ...
        kwargs_sig_list2 = [
            f"{pname}={val}"
            for pname, val in _get_default_parameters(sigparams, ignore_kwargs)
        ]
        kwargs_sig_list = (
            kwargs_sig_list
            + (["*"] if len(kwargs_sig_list2) > 0 else [])
            + kwargs_sig_list2
        )
