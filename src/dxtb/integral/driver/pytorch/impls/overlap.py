...
def _overlap(
    ...
    with profile(
        ...
        with record_function("create_cgtos"):
            alphas, coeffs = bas.create_cgtos()

        with record_function("spread_ushell_to_orbital"):
        ...
        with record_function("spread_atom_to_orbital"):
            pos = ihelp.spread_atom_to_orbital(positions, dim=-2, extra=True)

        with record_function("spread_shell_to_orbital"):
            ang = ihelp.spread_shell_to_orbital(ihelp.angular)

        with record_function("setup_cutoff"):
            if cutoff is None:
                mask = None
            else:
        ...
        with record_function("unique_shell_pairs"):
            umap, n_unique_pairs = bas.unique_shell_pairs(mask=mask, uplo=uplo)

        with record_function("initialize_overlap_matrix"):
            ovlp = torch.zeros(
                *umap.shape, dtype=positions.dtype, device=positions.device
            )

        with record_function("overlap_calculation"):
            for uval in range(n_unique_pairs):
                with record_function("pair_processing"):
                ...
                with record_function("basis_function_processing"):
                    alpha_tuple = (
        ...
        with record_function("finalize_overlap_matrix"):
            if uplo == "l":
