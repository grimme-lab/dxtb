...
class Packer:
    ...
    .. doctest::
        ...
        ...     "c": a,
        ... }
        >>> packer = dxtb._src.exlibs.xitorch.Packer(obj)
    ...
    def __init__(self, obj: Any):
        ...
        # caches
        self._params_tensor_list: list[torch.Tensor] | None = tensor_lists
        self._unique_params_idxs: list[int] | None = None
        self._unique_inverse_idxs: list[int] | None = None
        self._unique_tensor_shapes: list[torch.Size] | None = None
        self._tensor_shapes: list[torch.Size] | None = None
        self._unique_tensor_numels: list[int] | None = None
        self._unique_tensor_numel_tot: int | None = None
        self._tensor_numels: list[int] | None = None
        self._tensor_numel_tot: int | None = None
    ...
    def get_param_tensor(self, unique: bool = True) -> torch.Tensor | None:
        """
        ...
        ``dxtb._src.exlibs.xitorch.optimize.minimize``.
    ...
    def construct_from_tensor_list(
        self, tensors: list[torch.Tensor], unique: bool = True
    ) -> Any:
        """
        ...
        if tensor_shapes is None:
            ...
            for i, (tens, shape) in enumerate(zip(tensors, tensor_shapes)):
                if tens.shape != shape:
                    msg = (
                        "The tensors[%d] has mismatch shape from the original. Expected: %s, got: %s"
                        % (i, tens.shape, shape)
                    )
            ...
            if unique:
                ...
                tensors = [
                    tensors[self._unique_inverse_idxs[i]]
                    for i in range(len(self._unique_inverse_idxs))
                ]
            else:
    ...
    def construct_from_tensor(self, a: torch.Tensor, unique: bool = True) -> Any:
        ...
        if tensor_shapes is None:
            raise RuntimeError(
                "Please execute self.get_param_tensor(%s) first" % str(unique)
            )
            ...
            if len(tensor_numels) == 1:
                ...
                for i in range(len(tensor_numels)):
                    p = a[ioffset : ioffset + tensor_numels[i]].reshape(
                        tensor_shapes[i]
                    )
