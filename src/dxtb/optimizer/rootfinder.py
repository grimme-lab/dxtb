...
"""Assembly of different implementations for rootfinders.
For more details see:
    * LBFGS implementation: https://github.com/hjmshi/PyTorch-LBFGS
    * BROYDEN implementation: https://github.com/DiffEqML/torchdyn/blob/master/torchdyn/numerics/solvers/root.py
    * DEQ implementation: https://github.com/locuslab/deq
"""


def broyden_good(x, f_equations, J_equations, tol=10e-10, maxIters=50):
    # Thanks to: https://nickcdryan.com/2017/09/16/broydens-method-in-python/
    ...
    while tla.norm(f, 2) > tol and steps_taken < maxIters:
        s = tla.solve(J, -1 * f)
        ...
        J = J + (torch.outer((z - torch.matmul(J, s)), s)) / (torch.dot(s, s))
...
def anderson(
    ...
    **kwargs,
    ...
    """
    # Thanks to: https://github.com/mcbal/deep-implicit-attention/blob/main/deep_implicit_attention/solvers.py
...
def broyden2(f, x0, threshold, eps=1e-3, stop_mode="rel", ls=False, name="unknown"):
    ...
    Us = (
        torch.zeros(bsz, total_hsize, seq_len, threshold)
        .to(dev)
        .double()
        # .requires_grad_(True)
    )  # One can also use an L-BFGS scheme to further reduce memory
    VTs = (
        torch.zeros(bsz, threshold, total_hsize, seq_len)
        .to(dev)
        .double()
        # .requires_grad_(True)
    )
    ...
    while nstep < threshold:
        ...
        for mode in ["rel", "abs"]:
            if diff_dict[mode] < lowest_dict[mode]:
                if mode == stop_mode:
                    # that is not even the cause of the RAM leak!
        ...
        vT[vT != vT] = 0
        u[u != u] = 0
        VTs[:, nstep - 1] = vT  # self assignment --> bad for gradient
        Us[:, :, :, nstep - 1] = u
