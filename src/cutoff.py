...
def get_lattice_points_rep_3d(lat, rep, origin):
    ...
    if origin is not None:        
        trans = torch.zeros([3, torch.prod(2*rep+1)]) #TODO: allocate(trans(3, product(2*rep+1)))
        for ix in range(rep[1]+1):
            for iy in range(rep[2]+1):
                for iz in range(rep[3]+1):
                    for jx in range(torch.where(ix > 0, -1, 1), -2): # TODO: merge(-1, 1, ix > 0), -2
                        for jy in range(torch.where(iy > 0, -1, 1), -2):
                            for jz in range(torch.where(iz > 0, -1, 1), -2):
                                trans[:, itr] = lat[:, 0]*ix*jx + lat[:, 1]*iy*jy + lat[:, 2]*iz*jz
                                itr += 1
    else:
        trans = torch.zeros([3, torch.prod(2*rep+1)-1]) #TODO: allocate(trans(3, product(2*rep+1)-1))
        for ix in range(rep[1]+1):
            for iy in range(rep[2]+1):
                for iz in range(rep[3]+1):
...
def get_lattice_points_cutoff(periodic, lat, rthr, trans):
...
def get_translations(lat, rthr, rep):
